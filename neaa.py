...
tf.flags.DEFINE_string('output_dir', './adv/NAA-zaoshengqiwangscale1-vgg_16/', 'Output directory with images.')
...
def get_NAA_loss(opt_operations,weights,base_feature):
    ...
    for layer in opt_operations:
        ...
        # 获取张量 A 的形状的第一个维度大小
        size_first_dimension = tf.shape(base_feature)[0]
        size_first_dimension_adv_tensor = tf.shape(adv_tensor)[0]
        # 在第一个维度上复制
        adv_tensor = tf.tile( tf.expand_dims(adv_tensor,axis=0), [10,1,1,1,1])

        attribution = (adv_tensor-base_feature)*weights
        attribution = tf.reduce_sum(attribution, axis=0)
...
def interpolate_and_compose(a, b_small):
    # 将a转换为灰度图像
    gray_a = np.mean(a, axis=-1)

    # 将b的前两个维度通过线性插值扩展到与a相同的维度
    x = np.linspace(0, 1, b_small.shape[0])
    y = np.linspace(0, 1, b_small.shape[1])

    x_new = np.linspace(0, 1, a.shape[0])
    y_new = np.linspace(0, 1, a.shape[1])

    # 创建插值函数
    interp_func = interp2d(x, y, b_small, kind='linear')

    # 使用插值函数得到新的b
    b_large = interp_func(x_new, y_new)

    # 将b_large的值归一化到范围 [0, 1]
    b_large = np.abs(b_large) / np.max(np.abs(b_large))

    # 在灰度图像上增加绿色通道值，根据b_large
    c = np.stack([gray_a, gray_a + b_large * 255, gray_a], axis=-1)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        weights_ph = tf.placeholder(dtype=tf.float32, shape=(10,20,35,35,256))
        base_feature = tf.placeholder(dtype=tf.float32, shape=(10,20,35,35,256))
        ...
        with tf.Session() as sess:
            ...
            for images,names,labels in utils.load_image(FLAGS.input_dir, FLAGS.image_size,FLAGS.batch_size):
                ...
                naaweight_np = np.zeros(shape=shape)
                ...
                for i in range(num_iter):
                    if i==0:
                        ...
                        for n in range(10):
                            images_tmp2 = image_preprocessing_fn(np.copy(images))
                            x_base = images_tmp2 + np.random.normal(size=images.shape, loc=0.0, scale=1)
                            ...
                            feature_base = sess.run([opt_operations[0]],
                                                    feed_dict={ori_input: x_base, adv_input: x_base,label_ph: labels})
                            if n == 0:
                                weight_np_vertical = np.expand_dims(weight_np, axis=0)
                                feature_vertical = np.expand_dims(feature_base[0][:FLAGS.batch_size], axis=0)
                                print("feature_vertical", np.array(feature_vertical).shape)
                            else:
                                weight_np_vertical = np.vstack([weight_np_vertical, np.expand_dims(weight_np, axis=0)])
                                feature_vertical = np.vstack([feature_vertical, np.expand_dims(feature_base[0][:FLAGS.batch_size], axis=0)])
                                print("weight_np_vertical",weight_np_vertical.shape)
                                print("feature_vertical", feature_vertical.shape)

                            weight_np = np.zeros_like(weight_np)

                        # print(weight_np_vertical.shape)
                        for l in range(int(FLAGS.ens)):
                            x_base = np.array([0.0,0.0,0.0])
                            x_base = image_preprocessing_fn(x_base)
                            images_tmp2 = image_preprocessing_fn(np.copy(images))
                            ...
                            images_tmp2 = images_tmp2*(1 - l/FLAGS.ens)+ (l/FLAGS.ens)*x_base
                            w, feature = sess.run([weights_tensor, opt_operations[0]],feed_dict={ori_input: images_tmp2, adv_input: images_tmp2, label_ph: labels})
                            naaweight_np = naaweight_np + w[:FLAGS.batch_size]
                    ...
                    images_adv, grad_np, amplification_np=sess.run([adv_input_update, noise, amplification_update],
                                              feed_dict={ori_input:images_tmp,adv_input:images_adv,weights_ph:weight_np_vertical, base_feature: feature_vertical,
                    ...
                    if i == 0:
                        fmaaw = np.sum(weight_np_vertical,axis=0)[:,:,:,0]
                        naaw = naaweight_np[:,:,:,0]
                        for n in range(20):
                            result_image = interpolate_and_compose(images[n], fmaaw[n])
                            plt.imshow(result_image)
                            plt.savefig('./highlightslingyuguiyin-incv3/' + names[n], dpi=300)
                            result_image1 = interpolate_and_compose(images[n], naaw[n])
                            plt.imshow(result_image1)
                            plt.savefig('./highlightsnaa-incv3/'+names[n], dpi=300)
